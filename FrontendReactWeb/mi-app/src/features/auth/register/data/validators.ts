...
export function validateRegisterForm(v: RegisterPayload): ValidationErrors {
  ...
  else if (v.name.trim().length < 2) err.name = "Mínimo 2 caracteres";

  if (!v.login.trim()) err.login = "El usuario es requerido";
  else if (v.login.trim().length < 3) err.login = "Mínimo 3 caracteres";
  else if (!/^[a-zA-Z0-9._@-]+$/.test(v.login.trim()))
    err.login = "Solo letras, números y ._@-";

  if (!v.email.trim()) err.email = "El email es requerido";
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.email.trim()))
    err.email = "Email inválido";

  if (!v.password) err.password = "La contraseña es requerida";
  else if (v.password.length < 8) err.password = "Mínimo 8 caracteres";
