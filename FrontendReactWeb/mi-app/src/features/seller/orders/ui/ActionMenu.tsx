...
export default function ActionMenu({ order, onStatus, onDelete, onView }: Props) {
  ...
  const statusActions: StatusAction[] = (
    [
      order.status !== "sent"      && { label: "📦 Marcar como enviado",   next: "sent"      as OrderStatus, color: T.brand600 },
      order.status !== "delivered" && { label: "✅ Marcar como entregado", next: "delivered" as OrderStatus, color: T.success  },
      order.status !== "pending"   && { label: "🔄 Volver a pendiente",    next: "pending"   as OrderStatus, color: T.warn     },
      order.status !== "cancelled" && { label: "🚫 Cancelar envío",        next: "cancelled" as OrderStatus, color: T.error    },
