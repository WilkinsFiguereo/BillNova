...
type NextAction = [string, OrderStatus, string];
...
export default function DetailModal({ order, onClose, onStatus }: Props) {
  ...
  const nextActions: NextAction[] = (
    [
      order.status !== "sent"      && ["📦 Marcar enviado",   "sent"      as OrderStatus, T.brand600],
      order.status !== "delivered" && ["✅ Marcar entregado", "delivered" as OrderStatus, T.success ],
      order.status !== "cancelled" && ["🚫 Cancelar envío",   "cancelled" as OrderStatus, T.error   ],
    ] as (NextAction | false)[]
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          {nextActions.length > 0 && (
            <div style={{ marginTop: 20, display: "flex", gap: 8, flexWrap: "wrap" }}>
              {nextActions.map(([label, st, color]) => (
