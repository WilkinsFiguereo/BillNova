...
export function DashboardModPage() {
  const { data, isLoading, isRefreshing, periodo, setPeriodo, ahora, refresh } = useDashboard();
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: colors.bg.primary, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <main style={{ flex: 1, padding: '24px 22px', overflow: 'auto' }}>
        <DashboardTopBar ahora={ahora} periodo={periodo} onPeriodo={setPeriodo} onRefresh={refresh} isRefreshing={isRefreshing} />
