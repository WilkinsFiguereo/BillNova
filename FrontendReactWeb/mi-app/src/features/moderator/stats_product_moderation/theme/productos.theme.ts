"use client";

import { CategoriaProducto, EstadoProducto } from '../types/productos.types';

export const colors = {
  brand:  { 600: '#1E3A8A', 700: '#1A2F73', 400: '#3B82F6', 100: '#DBEAFE' },
  bg:     { primary: '#F8FAFC', secondary: '#FFFFFF', alt: '#F1F5F9' },
  text:   { primary: '#1F2937', secondary: '#4B5563', disabled: '#9CA3AF' },
  estado: {
    success: { main: '#10B981', bg: '#D1FAE5', text: '#065F46' },
    error:   { main: '#EF4444', bg: '#FEE2E2', text: '#991B1B' },
    warning: { main: '#F59E0B', bg: '#FEF3C7', text: '#92400E' },
    info:    { main: '#3B82F6', bg: '#DBEAFE', text: '#1E40AF' },
    neutral: { main: '#9CA3AF', bg: '#F3F4F6', text: '#374151' },
    purple:  { main: '#7C3AED', bg: '#EDE9FE', text: '#5B21B6' },
  },
  border: '#E2E8F0',
  shadow: 'rgba(30, 58, 138, 0.08)',
} as const;

export const estadoProductoConfig: Record<
  EstadoProducto,
  { label: string; color: (typeof colors.estado)[keyof typeof colors.estado] }
> = {
  activo:     { label: 'Activo',     color: colors.estado.success },
  agotado:    { label: 'Agotado',    color: colors.estado.warning },
  suspendido: { label: 'Suspendido', color: colors.estado.error   },
  borrador:   { label: 'Borrador',   color: colors.estado.neutral },
};

export const categoriaConfig: Record<CategoriaProducto, { label: string; color: string }> = {
  electronica:  { label: 'Electrónica',  color: '#1E3A8A' },
  moda:         { label: 'Moda',         color: '#7C3AED' },
  hogar:        { label: 'Hogar',        color: '#10B981' },
  alimentos:    { label: 'Alimentos',    color: '#EF4444' },
  deportes:     { label: 'Deportes',     color: '#F59E0B' },
  salud:        { label: 'Salud',        color: '#EC4899' },
  tecnologia:   { label: 'Tecnología',   color: '#0EA5E9' },
  belleza:      { label: 'Belleza',      color: '#A855F7' },
  otro:         { label: 'Otro',         color: '#9CA3AF' },
};

export const ordenConfig = {
  ventas:      'Total Ventas',
  ingresos:    'Ingresos',
  calificacion:'Calificación',
  crecimiento: 'Crecimiento',
  vistas:      'Vistas',
  devolucion:  'Devolución',
} as const;

export const periodoConfig = {
  '7d':  'Últimos 7 días',
  '30d': 'Últimos 30 días',
  '90d': 'Últimos 90 días',
  '1y':  'Último año',
} as const;