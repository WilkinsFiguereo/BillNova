...
function getFallbackConfigHref(from: string) {
  if (from === "admin") return "/navigation/config/page";
  if (from === "moderation" || from === "moderator") return "/navigation/config/page";
  if (from === "seller") return "/navigation/config/page";

  const user = getStoredAuthState();
  const role = user?.role ?? null;
  if (role === "admin" || role === "moderator" || role === "seller" || role === "gerente" || role === "worker") {
    return "/navigation/config/page";
  }
